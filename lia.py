...
def banner():
    print("""
\x1b[0;91m.____     .__         Team?Notツ
\x1b[0;91m|    |    |__|_____   Free Not Buyツ
\x1b[0;91m|    |    |  |\__  \  Recode>LiaCansツ
\x1b[0;97m|    |___ |  | / __ \_Author>Dapunta-Rizalツ
\x1b[0;97m|_______ \|__|(____  /I`m Okeyツ
        \x1b[0;97m\/         \/Github.com/LiaCans
""")
def menu_log():
...
def menu():
    ...
    print('%s►%s►%s► 1. %sCrack From Public'%(B,H,M,O))
    print('%s►%s►%s► 2. %sCrack From Follower'%(B,H,M,O))
    print('%s►%s►%s► 3. %sCrack From likes post'%(B,H,M,O))
    print('%s►%s►%s► 4. %sMengambil Data Target'%(B,H,M,O))
    print('%s►%s►%s► 5. %sDump ID '%(B,H,M,O))
    print('%s►%s►%s► 6. %sCheck Crack Results'%(B,H,M,O))
    print('%s►%s►%s► 7. %sCheck Crack Result Options '%(O,P,O,P))
    print('%s►%s►%s► 8. %sUser Agent %sU%s/%sA'%(B,H,M,O,U,P,O))
    #print('%s►%s►%s► 9. %sCrack From Nomber'%(O,P,O,P))
    #print('%s►%s►%s► 10. %sCrack Instagram'%(B,H,M,O))
    print('%s►%s►%s► 0. %sLog Out'%(O,P,O,P))
...
def cek_hasil():
    ...
    menu()
def var_menu():
    print('%s►%s►%s►  %sSelect Method Login'%(B,H,M,O))
    print('%s►%s►%s► 1. %sLogin with Token'%(B,H,M,O))
    print('%s►%s►%s► 2. %sLogin with Cookies'%(B,H,M,O))
    #print('%s►%s►%s► 4. %sScript Usage Tutorial'%(B,H,M,O))
    #print('%s►%s►%s► 5. %sInfo Author & Team Project'%(B,H,M,O))
    print('%s►%s►%s► 0. %sGo Back'%(B,H,M,O))
def var_ugen():
    print("%s►%s►%s► 1. %sGet User Agent"%(B,H,M,O))
    print("%s►%s►%s► 2. %sChange User Agent%s(%sManual%s)"%(B,H,M,U,O,H,O))
    print("%s►%s►%s► 3. %sChange User Agent %s(%sAdjust HP%s)"%(B,H,M,U,O,H,O))
    print("%s►%s►%s► 4. %sDelete User Agent"%(B,H,M,O))
    print("%s►%s►%s► 5. %sCheck User Agent"%(B,H,M,O))
    print("%s►%s►%s► %sReturn"%(B,H,M,O))
def start_method():
    print('%s►%s►%s► 1. %sMethod Api'%(B,H,M,O))
    print('%s►%s►%s► 2. %sMethod Mbasic'%(B,H,M,O))
    print('%s►%s►%s► 3. %sMethod Free FB'%(B,H,M,O))
def start_methodezz():
    print('%s►%s►%s► 1. %sFast Crack %s[%s6 pass%s]'%(B,H,M,U,O,K,O))
    print('%s►%s►%s► 2. %sSlow Crack %s[%s9 pass%s]'%(B,H,M,U,O,K,O))
    print('%s►%s►%s► 3. %sVery Slow Crack %s[%s12 pass%s]'%(B,H,M,U,O,K,O))
    print('%s►%s►%s► %sCrack Password Combine'%(B,H,M,O))
def started():
    print('%s►%s► %sCrack Is Running...'%(B,H,O))
    print('%s►%s► %sAccount [OK] Saved To OK/%s.txt'%(B,H,O,tanggal))
    print('%s►%s► %sAccount [CP] Saved To CP/%s.txt'%(B,H,O,tanggal))
    print('%s►%s► %sActivate Airplane Mode [5 Seconds Only] Every 5 Minutes\n'%(B,H,O))
def folder():
    try:os.mkdir("CP")
    except:pass
    try:os.mkdir("OK")
